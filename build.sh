...
if [ -n "$KERNEL_NAME" ]; then
    sed -i "s/CONFIG_LOCALVERSION=\".*\"/CONFIG_LOCALVERSION=\"$KERNEL_NAME\"/g" arch/arm64/configs/genesis_defconfig
fi

if [ "$SPOOF_UNAME" == "on" ]; then
    # Spoof to standard Android stock naming (remove custom localversion)
    sed -i 's/CONFIG_LOCALVERSION=.*/CONFIG_LOCALVERSION=\"\"/g' arch/arm64/configs/genesis_defconfig
fi
...
# ==========================================
mkdir -p "$OUT_DIR"
make O="$OUT_DIR" CC=clang LLVM=1 LLVM_IAS=1 KCFLAGS="$KERNEL_KCFLAGS" LDFLAGS="$KERNEL_LDFLAGS" genesis_defconfig || exit 1
...
# Package
# ==========================================
find "$KERNEL_DIR" -maxdepth 1 -type f -name "genesis-*.zip" -exec rm -v {} \;
rm -rf "$KERNEL_DIR/genesis-Release"
...
if [ "$SPOOF_UNAME" == "on" ]; then
    ...
    sed -i "s/# CONFIG_KSU_SUSFS_SPOOF_UNAME is not set/CONFIG_KSU_SUSFS_SPOOF_UNAME=y/g" arch/arm64/configs/genesis_defconfig
elif [ "$SPOOF_UNAME" == "off" ]; then
    sed -i "s/CONFIG_KSU_SUSFS_SPOOF_UNAME=y/# CONFIG_KSU_SUSFS_SPOOF_UNAME is not set/g" arch/arm64/configs/genesis_defconfig
fi
...
ZIP_NAME="genesis-Bomb-${VERSION}${ZIP_SUFFIX}${HZ_LABEL}-$TIME.zip"
cd "$TEMP_DIR" && zip -r9 "../$ZIP_NAME" * -x .git README.md *placeholder > /dev/null && cd ..
rm -rf "$TEMP_DIR"

# Copy to release dir for CI
mkdir -p "$KERNEL_DIR/genesis-Release"
cp "$KERNEL_DIR/$ZIP_NAME" "$KERNEL_DIR/genesis-Release/"
